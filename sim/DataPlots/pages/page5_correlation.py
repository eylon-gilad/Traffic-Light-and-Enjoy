...
layout = html.Div([
    ...
    html.P("A heatmap showing the distribution between total cars in sim and average velocity."),
...
@callback(Output("correlation-heatmap", "figure"),
...
def update_correlation_heatmap(n_intervals):
    ...
    if (not total_cars_history or not avg_velocity_history or
            len(total_cars_history) != len(avg_velocity_history)):
        empty_df = pd.DataFrame({'cars': [], 'avg_vel': []})
        return px.scatter(empty_df, x='cars', y='avg_vel',
