...
@callback(
    Output("car-time-histogram", "figure"), Input("interval-component", "n_intervals")
)
def update_car_time_histogram(_):
    ...
    if not car_time_in_sim:
        ...
        fig.update_layout(
            xaxis_title="Time in Simulation (ms)", yaxis_title="Number of Cars"
        )
        return fig
    ...
    fig = px.histogram(
        df, x="time_in_sim", nbins=20, title="Distribution of Car Times in Simulation"
    )
    fig.update_layout(
        xaxis_title="Time in Simulation (ms)", yaxis_title="Number of Cars"
    )
    fig.update_traces(hovertemplate="Time in Sim: %{x} ms<br>Count: %{y}")
    return fig
