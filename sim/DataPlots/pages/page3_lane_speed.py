...
def update_lane_speed_charts(_):
    ...
    for lane_id, speeds in lane_avg_speeds_history.items():
        ...
        df = pd.DataFrame({"iteration": range(len(speeds)), "avg_speed": speeds})
        ...
        fig.update_layout(xaxis_title="Iteration", yaxis_title="Speed (units/s)")
        fig.update_traces(hovertemplate="Iteration: %{x}<br>Speed: %{y}")

        children.append(dcc.Graph(figure=fig, id=f"lane-speed-plot-lane-{lane_id}"))
