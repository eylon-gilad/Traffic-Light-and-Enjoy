...
class Sim:
    ...
    def get_random_directions(self, lane_index: int, lane_amount: int = 3):
        road_id = road_id = lane_index // 10 % 10
        options = self.all_car_directions(lane_index, lane_amount)
        options = self.remove_no_lights_paths(road_id, options)
        if len(options) == 0:
            return road_id
        return options[random.randint(0, len(options) - 1)]

    def remove_no_lights_paths(self, road_id: int, options: list[int], junction_id: int = 1):
    ...
    @staticmethod
    def all_car_directions(lane_index: int, lane_amount: int = 3):
        """
        creates all the possible directions for the car acording to the lane
        :param lane_index: the full id of the lane(consists also the road id)
        :param lane_amount: the amount of lanes in the road
        """
        l_transform = [3, 2, 0, 1]
        r_transform = [2, 3, 1, 0]
        road_id = lane_index // 10 % 10
        lane_id = lane_index % 10
        division_lane = lane_amount // 3
        options = []
