...
class SmartTrafficController(BaseAlgorithm):
    ...
    def __init__(
        ...
        self.car_wait_times: Dict[Tuple[int, ...], Dict[int, List[float]]] = (
            defaultdict(dict)
        )
        ...
        self.last_served_time: Dict[Tuple[int, ...], float] = {
            comb: current_time for comb in self.combinations
        }
                ...
                if scores:
                    ...
                    if self.current_combination is not None:
                        ...
                        if (
                            ...
                            and (best_score - current_score) < self.hysteresis_threshold
                        ):
                            best_combination = self.current_combination
                    ...
                    if best_combination != self.current_combination:
                        logging.info(
                            f"Switching to combination {best_combination} with score {best_score:.2f}"
                        )
    ...
    def _update_car_wait_times(self) -> None:
        ...
        for comb, car_dict in self.car_wait_times.items():
            ...
            self.car_wait_times[comb] = {
                cid: info for cid, info in car_dict.items() if cid in current_car_ids
            }
        ...
        for comb in self.combinations:
            ...
            for tl_id in comb:
                ...
                origin_lane_ids = (
                    tl.get_origins()
                )  # lanes from which cars go through this traffic light
                road_id = (
                    origin_lane_ids[0] // 10
                )  # derive the road id from the lane (assuming lane ID encoding)
                ...
                for lane in road.get_lanes_by_ids(origin_lane_ids):
                    for car in lane.get_cars():
                        ...
                        if cid not in self.car_wait_times[comb]:
                            ...
                            self.car_wait_times[comb][cid][0] = (
